$(function() {
    var len = $(".list").length;
    for (var i = 0; i < 2; i++) {
        var pageDiv1 = new jsPage("list", i, "all", "p1", "1");
...
function ready2go() {
    ...
    //     alert(obj[i]);
    // }
    var pageNum = this.page_objq.find("a");
    $(pageNum).bind("click", function() { //点击页码的时候跳到相应页 
...
function jsPage(list_id, index, list_class, page_obj_id, pagesize) {
    // list_id 结果集UL的id
    // list_class 要显示的类别
    // page_id 存放页码的id
    // pagesize 每页显示多少条
    this.list_obj_id = list_id;
    this.list_Obj = $("." + list_id).eq(index);
    this.li = this.list_Obj.find("li");
    this.li.hide();
    this.page_obj_id = page_obj_id;
    var page = $("." + page_obj_id); //存放页码的div
    this.page_obj = page[index];
    this.page_objq = page.eq(index);
...
jsPage.prototype.page = function() {
    ...
    this.page_obj.innerHTML = "<div><span  id='info' >" + this.outstr + "<\/div>";
    this.outstr = "";
}